        ...
        <Copy animateOnScroll={false} delay={0.2}>
          <Link
            ...
            className="text-[#fcf2e8] text-xl md:text-2xl font-bold justify-center
                  px-4 items-center cursor-pointer h-12 flex overflow-hidden rounded-2xl 
                  bg-[#141414] will-change-[height,border-radius] border border-[#333]
                  transition-colors duration-300"
            onMouseEnter={(e) => {
              // cycle through footer hover colors
              const colors = ["#c5ffc9", "#f9ffa5", "#feaac0", "#dcd0fe"];
              const randomColor = colors[Math.floor(Math.random() * colors.length)];
              (e.currentTarget as HTMLElement).style.backgroundColor = randomColor;
              (e.currentTarget as HTMLElement).style.color = "#141414"; // change text color for contrast
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.backgroundColor = "#141414";
              (e.currentTarget as HTMLElement).style.color = "#fcf2e8";
            }}
          >
            SANKALP
          </Link>
